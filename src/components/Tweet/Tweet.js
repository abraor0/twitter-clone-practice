...
function formatDate(dateInSec) {
  ...
  if (tweetYear !== todayYear) {
    formatedDate = `${monthNames[tweetDate.getMonth()]} ${tweetDate.getDay()}, ${tweetYear}`;
  } else {
...
const Tweet = ({ user_m_name, user_name, user_profile_image, created, text, ...toolbarData }) => {
    ...
    <article className="flex gap-x-3 border-y border-y-gray-100 p-3 pb-[6px] text-[15px]">
      ...
      <div className="flex-1">
        <header className="flex gap-x-2 items-center">
          <h3 className="font-bold"><a href="#">{user_name}</a></h3>
          <span className="text-gray-500">{`@${user_m_name}`} · {formatDate(created.seconds)}</span>
          <button className="ml-auto transition-colors flex text-gray-500 hover:text-sky-500 items-center gap-x-[6px] pr-2 group" type="button">
            <i className="text-xl p-2 group-hover:bg-sky-100 rounded-full relative">
              <Dots />
              <TweetToolbarTooltip text={'More'} />
            </i>
          </button>
        </header>
